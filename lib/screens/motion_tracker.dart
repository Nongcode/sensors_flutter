  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<UserAccelerometerEvent>(
        // Nếu máy ảo lỗi, hãy đổi thành accelerometerEventStream()
        stream: userAccelerometerEventStream(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return const Center(child: Text("Đang chờ dữ liệu...\n(Hãy lắc máy ảo)", textAlign: TextAlign.center, style: TextStyle(color: Colors.white)));
          }
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text("SHAKE COUNT: $_shakeCount", style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Colors.white)),
                Text("Gia tốc: ${acceleration.toStringAsFixed(2)} m/s²", style: const TextStyle(color: Colors.white70)),
              ],
            ),
          );
        },
